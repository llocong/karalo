...
class SearchTopUpTest {
    ...
    fun `fetches one more page when too few results pass`() {
        ...
        val result =
            filterWithOneTopUp(listOf(1, 2, 3), isEven, minResults = 3) {
                fetches++
                listOf(4, 5, 6)
            }
    ...
    fun `never fetches more than one extra page, even if results stay few`() {
        ...
        val result =
            filterWithOneTopUp(listOf(1, 3), isEven, minResults = 10) {
                fetches++
                listOf(5, 8)
            }
    ...
    fun `does not fetch another page when enough results already pass`() {
        ...
        val result =
            filterWithOneTopUp(listOf(2, 4, 5), isEven, minResults = 2) {
                fetches++
                listOf(6)
            }
