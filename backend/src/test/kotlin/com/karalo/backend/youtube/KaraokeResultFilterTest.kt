...
/** Same cases as the TV's `KaraokeResultFilterTest`, so the two copies of the rule stay in sync. */
class KaraokeResultFilterTest {
    ...
    ) = Item(title, channelName, durationSeconds)

    private fun keeps(
        item: Item,
        rawQuery: String = "machine",
    ) = isLikelyKaraoke(item.title, item.channelName, item.durationSeconds, rawQuery)
