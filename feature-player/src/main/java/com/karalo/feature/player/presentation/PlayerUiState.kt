...
data class PlayerUiState(
    ...
    /** Whether the persistent remote queue has anything after the current item -- drives the
     *  on-screen Next button's enabled state, same rule the webapp's own Skip button follows. */
    val hasNextInQueue: Boolean = false,
)
